...
namespace Jovian.ClientMap.classes
{
    ...
    public class XmlHelper
    {
            ...
            try
            {
                ...
                if (xmlNode!=null)
                {
                    ...
                    foreach (RemoteWin rw in rws)
                    {
                        ...
                        XmlElement img = xmlDoc.CreateElement("Img"); img.InnerText = rw.Img; modular.AppendChild(img);
        ...
        /// <param name="xPath"></param>
        /// <returns></returns>
        public static string[] GetScreenInfo(string xmlFileName, string xPath)
        {
            XmlDocument xmlDoc = new XmlDocument();
            ...
            {
                xmlDoc.Load(xmlFileName);
                XmlNode xmlNodes = xmlDoc.SelectSingleNode(xPath);
                if (xmlNodes != null)
                {
                    foreach (XmlElement xe in xmlNodes)
                    {
                        alInfo.Add(xe.InnerText);
                    }
                }
                return (string[])alInfo.ToArray(typeof(string));
            }
            catch (Exception ex)
            {
                LogHelper.WriteLog(ex.Message);
