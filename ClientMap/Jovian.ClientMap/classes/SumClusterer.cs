...
namespace Jovian.ClientMap.classes
{
    ...
    public class SumClusterer:GraphicsClusterer
    {
        ...
        protected override Graphic OnCreateGraphic(GraphicCollection cluster, MapPoint center, int maxClusterCount)
        {
            ...
            double size = 60;//(Math.Log(sum * SymbolScale / 10) * 10 + 20);
        ...
        private static void GetSizeAndColorByCount(double count, ref double size, ref Brush color)
        {
            if (count >= 0 && count <= 5)
            {
                size = 40; color = new SolidColorBrush(Color.FromArgb(200, 0x00, 0xa8, 0x84));
            }
            else if (count >= 6 && count <= 15)
            {
                size = 45; color = new SolidColorBrush(Color.FromArgb(200, 0x00, 0x90, 0x4a));
            }
            else if (count >= 16 && count <= 35)
            {
                size = 50; color = new SolidColorBrush(Color.FromArgb(200, 0x7b, 0xc9, 0x38));
            }
            else if (count >= 36 && count <= 55)
            {
                size = 55; color = new SolidColorBrush(Color.FromArgb(200, 0xfe, 0xf8, 0x02));
            }
            else if (count >= 56 && count <= 155)
            {
                size = 60; color = new SolidColorBrush(Color.FromArgb(200, 0xeb, 0xb7, 0x01));
            }
            else if (count >= 156 && count <= 555)
            {
                size = 65; color = new SolidColorBrush(Color.FromArgb(200, 0xf2, 0x96, 0x31));
            }
            else if (count >= 556 && count <= 1555)
            {
                size = 70; color = new SolidColorBrush(Color.FromArgb(200, 0xf3, 0x80, 0x01));
            }
            else
            {
                size = 80; color = new SolidColorBrush(Color.FromArgb(200, 0xeb, 0x3d, 0x00));
            }
        }
    ...
    internal class ClusterSymbol : MarkerSymbol
    {
        public ClusterSymbol()
        {
            ...
            <TextBlock 
                ...
                FontSize=""16"" Margin=""1,1,0,0"" FontWeight=""Bold""
                Foreground=""#99000000"" />
            <TextBlock
                Text=""{Binding Attributes[Count]}"" 
                FontSize=""16"" Margin=""0,0,1,1"" FontWeight=""Bold""
                Foreground=""White"" />
            </Grid>
            </Grid>
            </ControlTemplate>";
